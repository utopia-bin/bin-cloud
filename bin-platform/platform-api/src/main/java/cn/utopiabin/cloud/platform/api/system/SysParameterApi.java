...
public interface SysParameterApi {
    ...
    void create(@Parameter(description = "参数新增参数", required = true) SysParameterCreateDTO dto);

    @Operation(summary = "编辑参数", description = "按ID编辑参数，并刷新缓存")
    void update(@Parameter(description = "参数编辑参数", required = true) SysParameterUpdateDTO dto);

    @Operation(summary = "删除参数", description = "删除参数并清除对应缓存")
    void remove(@Parameter(description = "参数ID", required = true) Long id);

    @Operation(summary = "分页查询参数")
    PageResult<SysParameterVO> page(@Parameter(description = "分页查询条件", required = true) SysParameterPageQuery query);

    @Operation(summary = "列表查询参数")
    List<SysParameterVO> list(@Parameter(description = "列表查询条件") SysParameterListQuery query);

    @Operation(summary = "按key获取参数值", description = "从缓存获取，缓存不存在则查库并缓存")
    String getValue(
            @Parameter(description = "参数键", required = true) String key,
            @Parameter(description = "默认值，缓存和数据库均不存在时返回") String defaultValue);
