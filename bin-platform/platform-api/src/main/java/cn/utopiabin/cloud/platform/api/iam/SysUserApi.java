...
public interface SysUserApi {
    ...
    Long create(@Parameter(description = "用户新增参数", required = true) @Valid SysUserCreateDTO dto) throws BizException;

    @Operation(summary = "编辑用户", description = "按ID编辑用户信息，密码不填则不修改")
    void update(@Parameter(description = "用户编辑参数", required = true) @Valid SysUserUpdateDTO dto) throws BizException;

    @Operation(summary = "删除用户", description = "逻辑删除，同时删除该用户的所有角色关联")
    void remove(@Parameter(description = "用户ID", required = true) Long id) throws BizException;
    ...
    SysUserVO get(@Parameter(description = "用户ID", required = true) Long id) throws BizException;

    @Operation(summary = "分页查询用户")
    PageResult<SysUserVO> page(@Parameter(description = "分页查询条件", required = true) @Valid SysUserPageQuery query) throws BizException;

    @Operation(summary = "列表查询用户")
    List<SysUserVO> list(@Parameter(description = "列表查询条件") SysUserListQuery query) throws BizException;

    @Operation(summary = "为用户分配角色", description = "全量替换用户的角色列表")
    void assignRoles(@Parameter(description = "用户分配角色参数", required = true) @Valid SysUserAssignRolesDTO dto) throws BizException;

    @Operation(summary = "获取用户拥有的角色列表")
    List<SysRoleVO> getRoles(@Parameter(description = "用户ID", required = true) Long userId) throws BizException;

    @Operation(summary = "检查用户名是否存在", description = "用于新增/编辑时的唯一性校验")
    boolean existsByUsername(@Parameter(description = "用户名", required = true) String username) throws BizException;
    ...
    void resetPassword(@Parameter(description = "用户ID", required = true) Long userId,
                       @Parameter(description = "新密码", required = true) String newPassword) throws BizException;
}
