...
public interface SysRoleApi {
    ...
    Long create(@Parameter(description = "角色新增参数", required = true) @Valid SysRoleCreateDTO dto);

    @Operation(summary = "编辑角色", description = "按ID编辑角色信息")
    void update(@Parameter(description = "角色编辑参数", required = true) @Valid SysRoleUpdateDTO dto);
    ...
    PageResult<SysRoleVO> page(@Parameter(description = "分页查询条件", required = true) @Valid SysRolePageQuery query);
    ...
    void assignPermissions(
            @Parameter(description = "角色分配权限参数", required = true) @Valid SysRoleAssignPermissionsDTO dto);
