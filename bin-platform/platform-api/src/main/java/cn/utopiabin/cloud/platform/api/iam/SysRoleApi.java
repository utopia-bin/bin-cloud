...
public interface SysRoleApi {
    ...
    Long create(@Parameter(description = "角色新增参数", required = true) @Valid SysRoleCreateDTO dto) throws BizException;

    @Operation(summary = "编辑角色", description = "按ID编辑角色信息")
    void update(@Parameter(description = "角色编辑参数", required = true) @Valid SysRoleUpdateDTO dto) throws BizException;

    @Operation(summary = "删除角色", description = "同时删除该角色的所有菜单关联和用户关联")
    void remove(@Parameter(description = "角色ID", required = true) Long id) throws BizException;
    ...
    SysRoleVO get(@Parameter(description = "角色ID", required = true) Long id) throws BizException;

    @Operation(summary = "分页查询角色")
    PageResult<SysRoleVO> page(@Parameter(description = "分页查询条件", required = true) @Valid SysRolePageQuery query) throws BizException;

    @Operation(summary = "列表查询角色")
    List<SysRoleVO> list(@Parameter(description = "列表查询条件") SysRoleListQuery query) throws BizException;

    @Operation(summary = "为角色分配权限", description = "全量替换角色权限并校验乐观锁版本")
    void assignPermissions(
            @Parameter(description = "角色分配权限参数", required = true) @Valid SysRoleAssignPermissionsDTO dto) throws BizException;

    @Operation(summary = "获取角色拥有的权限列表")
    List<SysPermissionVO> getPermissions(
            @Parameter(description = "角色ID", required = true) Long roleId) throws BizException;

    @Operation(summary = "检查角色编码是否存在", description = "用于新增/编辑时的唯一性校验")
    boolean existsByCode(@Parameter(description = "角色编码", required = true) String code) throws BizException;
}
