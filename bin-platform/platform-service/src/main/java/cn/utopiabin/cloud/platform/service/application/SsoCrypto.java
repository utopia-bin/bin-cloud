...
public final class SsoCrypto {
    ...
    private SsoCrypto() {}
    public static String random() { byte[] value = new byte[32]; RANDOM.nextBytes(value); return Base64.getUrlEncoder().withoutPadding().encodeToString(value); }
    ...
    public static String challenge(String value) { return Base64.getUrlEncoder().withoutPadding().encodeToString(digest(value)); }
    ...
    public static String redirect(String value, String environment) {
        try {
            ...
            if (uri.getHost() == null || uri.getUserInfo() != null || uri.getFragment() != null || uri.getRawQuery() != null
                    || !("https".equals(uri.getScheme()) || "DEV".equals(environment) && "http".equals(uri.getScheme()))
                    || !uri.normalize().equals(uri)) throw new IllegalArgumentException();
            return value;
        } catch (Exception e) { throw new BizException(400, "回调地址必须为完整精确地址，不能包含查询串或片段；本地HTTP请选择DEV"); }
    }
    public static void navigation(String value, boolean emptyAllowed) {
        if (emptyAllowed && (value == null || value.isEmpty())) return;
        try {
            URI uri = URI.create(value);
            if (value.startsWith("/") && !value.startsWith("//") && uri.getAuthority() == null && !value.contains("\\")) return;
            if (("http".equals(uri.getScheme()) || "https".equals(uri.getScheme())) && uri.getHost() != null && uri.getUserInfo() == null) return;
