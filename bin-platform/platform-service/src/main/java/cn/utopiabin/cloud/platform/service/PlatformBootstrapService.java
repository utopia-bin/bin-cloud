...
public class PlatformBootstrapService {
    ...
    public boolean initialize() {
        ...
        if (permission.get("tenant_id") != null
                || !flag(permission.get("available")) || flag(permission.get("is_delete"))) {
        ...
        if (password == null || password.length() < 12
            ...
            throw new IllegalStateException("请设置 PLATFORM_BOOTSTRAP_PASSWORD：至少 12 个字符，UTF-8 编码不超过 72 字节");
        }
        ...
        if (tenants.isEmpty()) {
            ...
            jdbc.update("INSERT INTO sys_tenant (id, name, code) VALUES (?, ?, ?)",
                    tenantId, tenantName.trim(), tenantCode);
        } else {
            ...
            LocalDateTime expiry = jdbc.queryForObject(
                    "SELECT expire_time FROM sys_tenant WHERE id = ?", LocalDateTime.class, tenant.get("id"));
        ...
        if (!jdbc.queryForList("SELECT id FROM sys_user WHERE tenant_id = ? AND username = ? FOR UPDATE",
    ...
    private void menu(String name, String path, String icon, String permission, int sort) {
        ...
        jdbc.update("""
                ...
                VALUES (?, 2, ?, ?, ?, ?, ?)
                """, IdWorker.getId(), name, path, icon, permission, sort);
    }
