package cn.utopiabin.cloud.platform.service.auth;
 ...
 * 认证授权服务
 *
 * <p>封装登录、登出、获取当前用户、修改密码等业务逻辑。
 *
 * <p>生产级安全能力:
 *
 * <ul>
 *   <li>登录失败计数 + 账号锁定 (Redis, 可配置阈值/时长/窗口)
 *   <li>登录失败递增延迟 (防高速撞库)
 *   <li>登录时租户状态校验 (禁用/过期的租户禁止登录)
 *   <li>修改密码强度校验
 *   <li>登录防重复提交
 *   <li>操作日志审计
 * </ul>
...
public class AuthService {
  ...
  private final ApplicationRevocationService sessionRevocations;
   ...
   * 账号密码登录
   *
   * <p>安全流程: 锁定检查 → 密码校验(失败计数/递增延迟/阈值锁定) → 账号状态 → 租户状态 → 失败记录清零 → 权限聚合(缓存) → 签发 Token。
   *
   * <p>性能优化: 租户与用户验证各 1 次 DB 查询 + 权限聚合 2 次 JOIN 查询 (缓存命中时 0 次)
   *
   * @param dto 登录参数
  ...
  @OperateLog(
      module = "认证管理",
      action = "用户登录",
      type = OperateType.AUTH,
      principalSpel = "#dto.username",
      maskParams = true)
  public LoginResultVO login(LoginDTO dto) {
    ...
    if (StrUtil.isBlank(tenantCode) || StrUtil.isBlank(username) || StrUtil.isBlank(password)) {
      throw new BizException(
          PlatformErrorCode.PASSWORD_EMPTY.getCode(), PlatformErrorCode.PASSWORD_EMPTY.getMsg());
    }
    ...
    if (user == null || !passwordEncoder.matches(password, user.getPassword())) {
      ...
      throw new BizException(
          PlatformErrorCode.PASSWORD_ERROR.getCode(), PlatformErrorCode.PASSWORD_ERROR.getMsg());
    }

    // 3. 账号状态校验
    if (!Boolean.TRUE.equals(user.getAvailable())) {
      throw new BizException(
          PlatformErrorCode.USER_DISABLED.getCode(), PlatformErrorCode.USER_DISABLED.getMsg());
    }
  ...
  public void resetPasswordByPhone(PhoneResetPasswordDTO dto) {
    ...
    user.setCredentialVersion(
        java.util.Optional.ofNullable(user.getCredentialVersion()).orElse(0) + 1);
   ...
   * 退出登录
   *
   * <p>将 Token 加入 Redis 黑名单，gateway 将拦截已注销的 Token。
   *
   ...
   * 获取当前登录用户完整信息
   *
   * <p>基于 UserContext 获取 userId，从缓存加载权限数据。
   *
  ...
  public void changePassword(ChangePasswordDTO dto) {
    ...
    if (!passwordEncoder.matches(dto.getOldPassword(), user.getPassword())) {
      throw new BizException(
          PlatformErrorCode.PASSWORD_WRONG.getCode(), PlatformErrorCode.PASSWORD_WRONG.getMsg());
    }
  ...
  private void assertNotLocked(String loginIdentity, String username) {
    if (redisClient.hasKey(LOGIN_LOCK_KEY + loginIdentity)) {
      ...
      throw new BizException(
          PlatformErrorCode.ACCOUNT_LOCKED.getCode(), PlatformErrorCode.ACCOUNT_LOCKED.getMsg());
    }
  }
  ...
  private void recordLoginFailure(String loginIdentity, String username) {
    ...
    if (count == 1L) {
      redisClient.expire(
          failKey, Duration.ofSeconds(loginSecurityProperties.getFailCountWindowSeconds()));
    }

    // 递增延迟: 失败次数越多等待越久 (封顶 maxDelayMs)，提高撞库成本
    long delay =
        Math.min(
            count * loginSecurityProperties.getDelayBaseMs(),
    ...
    if (count >= loginSecurityProperties.getMaxFailCount()) {
      ...
      log.warn("账号触发锁定保护: username={}, failCount={}, lockSeconds={}", username, count, lockSeconds);
    } else {
      log.info(
          "登录失败: username={}, failCount={}/{}, delay={}ms",
          username,
          count,
          loginSecurityProperties.getMaxFailCount(),
          delay);
    }
  }
  ...
  private void validateTenant(Tenant tenant) {
    if (tenant == null) {
      throw new BizException(
          PlatformErrorCode.TENANT_NOT_FOUND.getCode(),
          PlatformErrorCode.TENANT_NOT_FOUND.getMsg());
    }
    if (!Boolean.TRUE.equals(tenant.getAvailable())) {
      throw new BizException(
          PlatformErrorCode.TENANT_DISABLED.getCode(), PlatformErrorCode.TENANT_DISABLED.getMsg());
    }
    if (tenant.getExpireTime() != null && tenant.getExpireTime().isBefore(LocalDateTime.now())) {
      throw new BizException(
          PlatformErrorCode.TENANT_EXPIRED.getCode(), PlatformErrorCode.TENANT_EXPIRED.getMsg());
    }
  }

  private LoginResultVO issueLoginResult(SysUser user) {
    var perm = permissionService.getUserPermissions(user.getId());
    var roleCodes = perm.getRoles().stream().map(SysRoleVO::getCode).toList();
