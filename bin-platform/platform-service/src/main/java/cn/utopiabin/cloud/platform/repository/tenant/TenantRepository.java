...
public class TenantRepository extends BaseRepository<TenantMapper, Tenant> {
    ...
    return page(
        ...
        new LambdaQueryWrapper<Tenant>()
            .and(
                ...
                qw ->
                    qw.like(Tenant::getName, query.getKeyword())
                        .or()
  ...
  public Tenant lockByCodeIncludingDeleted(String code) {
    return baseMapper.selectByCodeIncludingDeletedForUpdate(code);
  }

  public void lockExisting(long tenantId) {
    Tenant tenant =
        getOne(
            new LambdaQueryWrapper<Tenant>()
                .select(Tenant::getId)
                ...
                .last("FOR UPDATE"));
    if (tenant == null) {
      throw new BizException(404, "租户不存在或已删除");
    }
  }

  public long insertInitialTenant(String name, String code) {
