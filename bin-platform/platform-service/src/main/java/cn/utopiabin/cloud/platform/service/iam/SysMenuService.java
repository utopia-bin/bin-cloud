...
public class SysMenuService {
  ...
  public void update(SysMenuUpdateDTO dto) {
    if (dto.getId().equals(dto.getParentId())) {
      throw new BizException(
          PlatformErrorCode.MENU_PARENT_SELF.getCode(),
          PlatformErrorCode.MENU_PARENT_SELF.getMsg());
    }

    var menu = menuRepository.getOrThrow(dto.getId());
    if (!Objects.equals(menu.getVersion(), dto.getExpectedVersion())) {
  ...
  public void remove(Long id) {
    ...
    if (menuRepository.hasChild(id)) {
      throw new BizException(
          PlatformErrorCode.MENU_HAS_CHILDREN.getCode(),
  ...
  public List<SysMenuVO> list(SysMenuListQuery query) {
    return menuRepository.list(query).stream().map(m -> m.copyTo(SysMenuVO.class)).toList();
  }
  ...
  public List<SysMenuVO> listByPermissionCodes(List<String> permissionCodes) {
    return menuRepository.listViewsByPermissionCodes(permissionCodes);
  }
}
