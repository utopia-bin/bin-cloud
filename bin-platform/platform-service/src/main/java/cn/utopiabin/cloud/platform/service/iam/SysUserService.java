 ...
 *
 * <ul>
 *   <li>内置管理员账号保护 (不可删除/禁用)
 *   <li>自我保护 (不可删除/禁用当前登录账号，防止锁死系统)
 *   <li>密码强度校验 (创建/编辑/重置密码)
 *   <li>防重复提交 (创建/编辑)
 *   <li>分布式锁 (分配角色/重置密码，防并发竞态)
 *   <li>缓存失效延迟到事务提交后执行 (避免脏缓存)
 *   <li>操作日志审计
 * </ul>
 *
 * @since 1.0
...
public class SysUserService {
  ...
  public Long create(SysUserCreateDTO dto) {
    ...
    if (StrUtil.isBlank(username) || StrUtil.isBlank(dto.getPassword())) {
      throw new BizException(PlatformErrorCode.BAD_REQUEST.getCode(), "用户名与密码不能为空");
    }
    if (userRepository.countByField(SysUser::getUsername, username, null) > 0) {
      throw new BizException(
          PlatformErrorCode.USER_DUPLICATE.getCode(), PlatformErrorCode.USER_DUPLICATE.getMsg());
    }
    var phone = StrUtil.defaultIfBlank(dto.getPhone(), "").trim();
    if (StrUtil.isNotBlank(phone)
        && userRepository.countByField(SysUser::getPhone, phone, null) > 0) {
      throw new BizException(
          PlatformErrorCode.PHONE_DUPLICATE.getCode(), PlatformErrorCode.PHONE_DUPLICATE.getMsg());
    }
  ...
  public void update(SysUserUpdateDTO dto) {
    ...
    if (StrUtil.isBlank(username)) {
      throw new BizException(PlatformErrorCode.BAD_REQUEST.getCode(), "用户名不能为空");
    }
    ...
    if (isBuiltInAdmin(user) && !user.getUsername().equals(username)) {
      throw new BizException(PlatformErrorCode.BUILT_IN_PROTECTED.getCode(), "内置管理员账号不允许修改用户名");
    }
  ...
  public void remove(Long id) {
    ...
    TransactionAfterCommitExecutor.afterCommit(() -> permissionService.evictUserPermission(id));
  }
  ...
  public void batchDelete(BatchDeleteDTO dto) {
    ...
    if (users.size() != dto.getIds().size()) {
      throw new BizException(
          PlatformErrorCode.USER_NOT_FOUND.getCode(), PlatformErrorCode.USER_NOT_FOUND.getMsg());
    }
  ...
  public PageResult<SysUserVO> page(SysUserPageQuery query) {
    ...
    var records = page.getRecords().stream().map(u -> u.copyTo(SysUserVO.class)).toList();
    return PageResult.of(page.getCurrent(), page.getSize(), page.getTotal(), records);
  }

  @RequirePermission("platform:user:read")
  public List<SysUserVO> list(SysUserListQuery query) {
    return userRepository.list(query).stream().map(u -> u.copyTo(SysUserVO.class)).toList();
  }
  ...
  public void assignRoles(SysUserAssignRolesDTO dto) {
    ...
    if (!Objects.equals(user.getVersion(), dto.getExpectedVersion())) {
      throw new BizException(PlatformErrorCode.CONFLICT.getCode(), "用户已被其他操作修改，请刷新后重试");
    }
    ...
    if (roles.size() != roleIds.size()
      ...
      throw new BizException(
          PlatformErrorCode.TENANT_RELATION_VIOLATION.getCode(), "角色不存在、已禁用或不属于当前租户");
    }
  ...
  public List<SysRoleVO> getRoles(Long userId) {
    return roleRepository.listViewsByUserId(userId);
  }
  ...
  private void assertNotSelf(Long targetUserId) {
    ...
    if (currentId != null && Objects.equals(currentId, String.valueOf(targetUserId))) {
      throw new BizException(
          PlatformErrorCode.CANNOT_OPERATE_SELF.getCode(),
  ...
  private void assertNotBuiltInAdmin(SysUser user) {
    if (isBuiltInAdmin(user)) {
      throw new BizException(
          PlatformErrorCode.BUILT_IN_PROTECTED.getCode(),
