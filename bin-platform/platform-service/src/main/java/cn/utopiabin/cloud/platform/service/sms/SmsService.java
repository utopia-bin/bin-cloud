package cn.utopiabin.cloud.platform.service.sms;
...
public class SmsService {
  ...
  @TenantIgnore
  @cn.utopiabin.cloud.platform.annotation.OperateLog(
      module = "短信认证",
      action = "发送验证码",
      type = cn.utopiabin.cloud.platform.annotation.OperateType.AUTH,
      maskParams = true)
  public void sendVerificationCode(SmsCodeSendDTO dto) {
    ...
    String provider =
        sender == null ? normalizeProvider(properties.getDefaultProvider()) : sender.provider();
          ...
          sender.send(
              new SmsSendCommand(
                  ...
                  Map.of(
                      ...
                      code,
                      "minutes",
                      String.valueOf(Math.max(1, properties.getCodeTtl().toMinutes())))));
      if (result == null) {
        result = SmsSendResult.failure("EMPTY_RESULT", "短信厂商未返回发送结果");
      }
    } catch (RuntimeException ex) {
      log.warn(
          "短信厂商调用异常: tenantId={}, scene={}, provider={}, exceptionType={}",
          tenant.getId(),
          dto.getScene(),
          sender.provider(),
