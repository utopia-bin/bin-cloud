...
public class SysRoleService {
  ...
  public Long create(SysRoleCreateDTO dto) {
    ...
    if (StrUtil.isBlank(code)) {
      throw new BizException(PlatformErrorCode.BAD_REQUEST.getCode(), "角色编码不能为空");
    }
    if (roleRepository.countByField(SysRole::getCode, code, null) > 0) {
      throw new BizException(
          PlatformErrorCode.ROLE_CODE_DUPLICATE.getCode(),
          PlatformErrorCode.ROLE_CODE_DUPLICATE.getMsg());
    }

    var role = dto.copyTo(SysRole.class);
    role.setTenantApplicationId(
        Long.valueOf(cn.utopiabin.cloud.common.context.UserContextHolder.getTenantId()));
  ...
  public void update(SysRoleUpdateDTO dto) {
    ...
    if (!Objects.equals(role.getVersion(), dto.getExpectedVersion())) {
      throw new BizException(
          PlatformErrorCode.ROLE_VERSION_CONFLICT.getCode(),
    ...
    if (isBuiltInSuperAdmin(role) && !PlatformConstants.SUPER_ADMIN_ROLE_CODE.equals(code)) {
      throw new BizException(PlatformErrorCode.BUILT_IN_PROTECTED.getCode(), "内置超级管理员角色不允许修改编码");
    }
  ...
  public void batchDelete(BatchDeleteDTO dto) {
    ...
    if (roles.size() != dto.getIds().size()) {
      throw new BizException(
          PlatformErrorCode.ROLE_NOT_FOUND.getCode(), PlatformErrorCode.ROLE_NOT_FOUND.getMsg());
    }
  ...
  public PageResult<SysRoleVO> page(SysRolePageQuery query) {
    ...
    var records = page.getRecords().stream().map(r -> r.copyTo(SysRoleVO.class)).toList();
    return PageResult.of(page.getCurrent(), page.getSize(), page.getTotal(), records);
  }

  @RequirePermission("platform:role:read")
  public List<SysRoleVO> list(SysRoleListQuery query) {
    return roleRepository.list(query).stream().map(r -> r.copyTo(SysRoleVO.class)).toList();
  }
  ...
  public void assignPermissions(SysRoleAssignPermissionsDTO dto) {
    ...
    if (permissions.size() != requestedIds.size()
      ...
      throw new BizException(PlatformErrorCode.PERMISSION_NOT_FOUND.getCode(), "权限不存在或已被禁用");
    }

    Long operator = Long.valueOf(cn.utopiabin.cloud.common.context.UserContextHolder.getUserId());
    if (permissions.stream()
        .anyMatch(p -> !permissionService.hasPermission(operator, p.getCode()))) {
  ...
  public List<SysPermissionVO> getPermissions(Long roleId) {
    ...
    return permissionRepository.listViewsByRoleIds(List.of(roleId));
  }
  ...
  private void assertNotBuiltInSuperAdmin(SysRole role) {
    if (isBuiltInSuperAdmin(role)) {
      throw new BizException(PlatformErrorCode.BUILT_IN_PROTECTED.getCode(), "内置超级管理员角色不允许此操作");
    }
  }
}
