...
public class ApplicationCatalogService {
  ...
  public ApplicationVO get(long id) {
    var vo = repository.get(id);
    if (vo == null) throw new BizException(404, "应用不存在");
    return vo;
  }
  ...
  public long save(ApplicationDTO dto) {
    ...
    if (dto.isSsoEnabled() && dto.getRedirectUris().stream().noneMatch(RedirectDTO::isAvailable))
    ...
    if (dto.getId() == null) {
      ...
      if (!old.getCode().equals(dto.getCode()) || !old.getServiceId().equals(dto.getServiceId()))
      ...
      requireSingleChange(
          repository.update(id, dto, boundary.userId(), requireVersion(dto.getExpectedVersion())));
  ...
  public void remove(long id, int version) {
    ...
    if (instances.countActiveInstances(id) > 0) {
  ...
  public ClientSecretVO rotate(long id, int version) {
    ...
    String clientId = repository.lockService(id);
