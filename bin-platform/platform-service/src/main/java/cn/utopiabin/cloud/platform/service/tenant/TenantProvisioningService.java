...
public class TenantProvisioningService {
    ...
    public void provision(Long tenantId, TenantAdminDTO dto) {
        ...
        // 锁定租户主记录，使并发重试串行执行，并同时覆盖逻辑删除标记。
        jdbc.queryForObject(
                "SELECT id FROM sys_tenant WHERE id = ? AND is_delete = 0 FOR UPDATE",
                Long.class,
                tenantId);
        if (!jdbc.queryForList(
                        "SELECT id FROM sys_role WHERE application_id=1 AND tenant_id = ? AND code"
                            + " = 'tenant_admin'",
                        tenantId)
                .isEmpty()) {
        ...
        if (!jdbc.queryForList(
                        "SELECT id FROM sys_user WHERE tenant_id = ? AND username = ? AND is_delete"
                            + " = 0",
                        tenantId,
                        username)
                .isEmpty()) {
        ...
        var permissions =
                jdbc.queryForList(
                        "SELECT id, code FROM sys_permission WHERE application_id=1 AND tenant_id"
                            + " IS NULL AND available = 1 AND is_delete = 0");
        var grants =
                permissions.stream()
                        .filter(p -> ADMIN_PERMISSIONS.contains(p.get("code")))
                        .toList();
        ...
        jdbc.update(
                "INSERT INTO sys_user (id, tenant_id, username, password, real_name) VALUES (?, ?,"
                    + " ?, ?, '租户管理员')",
                userId,
                tenantId,
                username,
        ...
        jdbc.update(
                "INSERT INTO sys_role (id, tenant_id, tenant_application_id, name, code,"
                    + " data_scope) VALUES (?, ?, ?, '租户管理员', 'tenant_admin', 1)",
                roleId,
                tenantId,
                tenantId);
        jdbc.update(
        ...
        for (var permission : grants) {
            jdbc.update(
                    "INSERT INTO sys_role_permission (id, tenant_id, tenant_application_id,"
