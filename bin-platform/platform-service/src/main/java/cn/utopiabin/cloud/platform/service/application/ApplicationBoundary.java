...
public class ApplicationBoundary {
    ...
    public boolean global() {
        return permissions.hasPermission(userId(), "platform:application:provision");
    }

    public void require(String action) {
        if (!permissions.hasPermission(userId(), "platform:application:" + action))
            throw new BizException(403, "缺少应用" + action + "权限");
    }

    public long queryTenant(Long requested) {
        long own = tenantId();
        if (requested != null && requested != own && !global())
    ...
    public Map<String, Object> instance(long id, String action) {
        require(action);
        var row = store.one("applicationBoundarySelect01", id);
        queryTenant(number(row, "tenant_id"));
        return row;
    }

    public Map<String, Object> access(long tenant, long user, long instance) {
        var row = store.one("applicationBoundarySelect02", user, instance, tenant);
        ...
        if ("ASSIGNED".equals(row.get("access_policy"))) {
            var grant = store.queryForList("applicationBoundarySelect03", tenant, instance, user);
            if (grant.isEmpty()
