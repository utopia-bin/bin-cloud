...
public class CircuitBreakerFallbackHandler {
    ...
    public Mono<RestResult<Map<String, String>>> fallback(@PathVariable String service) {
        log.warn("[CircuitBreaker] 服务熔断降级: service={}", service);
        return Mono.just(RestResult.ok(
                "服务 [" + service + "] 暂时不可用, 已触发熔断保护",
                Map.of("service", service, "status", "circuit_open")
        ));
    }
    ...
    public Mono<RestResult<String>> fallbackDefault() {
        log.warn("[CircuitBreaker] 通用熔断降级触发");
        return Mono.just(RestResult.ok("服务暂时不可用, 已触发熔断保护", null));
    }
}
